...
export class CdpRegistry {
    ...
    queryByArchetype(archetype) {
        return this.list().filter((c) => c.identity.archetype === archetype);
    }
    queryBySideEffect(level) {
        return this.list().filter((c) => c.runtime.side_effects.level === level);
    }
    /**
     * 紧凑模型可读 marker，形如：
     ...
     */
    decorateSchemas(cap, existingDescription) {
     ...
     */
    applyBindings(schemas, bindings) {
        const toolNameToCap = new Map();
        for (const [capId, toolName] of Object.entries(bindings)) {
            const cap = this.get(capId);
            if (cap)
                toolNameToCap.set(toolName, cap);
        }
