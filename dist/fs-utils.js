 ...
 */
export async function resolveSafe(root, relPath) {
    ...
    if (abs !== resolvedRoot && !abs.startsWith(resolvedRoot + sep)) {
        throw new CdpSecurityError(abs, `path escapes root (${resolvedRoot})`);
    }
    ...
    if (realAbs !== resolvedRoot && !realAbs.startsWith(resolvedRoot + sep)) {
        throw new CdpSecurityError(realAbs, `symlink target escapes root (${resolvedRoot})`);
    }
    return abs;
}
/**
 ...
 */
export async function readCdpFile(filePath) {
    const stat = await fs.stat(filePath);
    if (stat.size > MAX_FILE_BYTES) {
        throw new CdpSecurityError(filePath, `file too large (${stat.size} > ${MAX_FILE_BYTES})`);
    }
    return fs.readFile(filePath, 'utf8');
}
export async function walkCdpFiles(root, maxDepth = MAX_DEPTH, maxFiles = MAX_FILES) {
    const files = [];
    let truncated = false;
