...
const logWarn = (ctx, msg) => {
    if (ctx.logger?.warn)
        ...
        console.warn(`[cdp] ${msg}`);
};
/* ----------------------------- 纯函数投影 ----------------------------- */
/**
 * 纯函数：在已有 description 后追加 CDP marker，零副作用、可单测。
 */
export function decorateSchemas(cap, existingDescription) {
 ...
 */
export function attachToTools(ctx, registry, bindings) {
    ...
    for (const [capId, toolName] of Object.entries(bindings)) {
        ...
        const defObj = def;
        // 幂等守卫
        if (originalDescriptions.has(defObj))
            continue;
        if (def.description.includes(SENTINEL))
            continue; // 已改写
...
export async function apply(ctx, rawConfig) {
    ...
    if (!parsed.success) {
        throw new Error(`dsh-cdp-metadata 配置非法: ${parsed.error.issues
