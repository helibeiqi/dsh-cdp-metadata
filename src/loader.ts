 ...
 * 加载单个文件：解析 + L1 校验。
 */
async function loadOneFile(filePath: string): Promise<LoadedCapability> {
  ...
  if (!syntax.ok) {
    // 聚合为单个报告错误，便于单文件定位
    const agg = new Error(
      `L1 syntax errors: ${syntax.errors.map((x) => x.message).join('; ')}`
    );
...
async function loadDir(
  root: string,
  dirEntry: string
): Promise<{ items: LoadedCapability[]; truncated: boolean }> {
...
export async function loadSources(
  ...
  for (const entry of cfg.paths) {
    ...
    try {
      ...
      if (err instanceof CdpSecurityError) throw err;
    ...
    if (stat.isDirectory()) {
      ...
      if (dirResults.truncated) truncated = true;
    } else if (stat.isFile()) {
      try {
