...
/** 否定词前缀表（中英文）。用于"否定启发式"。 */
export const NEG: readonly string[] = [
  '不',
  '不能',
  '无法',
  '禁止',
 ...
 * 若本身无否定前缀，返回原归一化串。
 */
export function stripNegation(s: string): string {
  const n = normalize(s);
...
export function hasNegation(s: string): boolean {
  const n = normalize(s);
 ...
 */
export function checkConflicts(cap: CdpCapability): CdpValidationError[] {
  const errors: CdpValidationError[] = [];
  const can = cap.boundaries.can;
  const cannot = cap.boundaries.cannot;

  const normalizeList = (xs: string[]): string[] => xs.map(normalize);
  ...
  for (let i = 0; i < canN.length; i++) {
    for (let j = 0; j < cannotN.length; j++) {
      if (canN[i] === cannotN[j]) {
        errors.push(
          new CdpValidationError(
            `capability.boundaries.cannot[${j}]`,
            `与 can[${i}]("${can[i]}") 直接矛盾`,
            'L2'
          )
        );
 ...
 * (b) downstream_hints.if_tag 必须命中 semantic_tags。
 */
export function checkTagHints(cap: CdpCapability): CdpValidationError[] {
  const errors: CdpValidationError[] = [];
  ...
  cap.output.downstream_hints.forEach((hint, idx) => {
    if (!tags.includes(hint.if_tag)) {
      errors.push(
        new CdpValidationError(
          `capability.output.downstream_hints[${idx}].if_tag`,
          `未出现在 semantic_tags 中 ("${hint.if_tag}")`,
          'L2'
        )
      );
    }
 ...
 */
export function checkLatency(cap: CdpCapability): CdpValidationError[] {
  const errors: CdpValidationError[] = [];
  if (!latencyRegex.test(cap.runtime.cost.latency)) {
    errors.push(
      new CdpValidationError(
        'capability.runtime.cost.latency',
        `非法 latency 格式 "${cap.runtime.cost.latency}" (应为 <1s / 500ms / 2m / 1h)`,
        'L2'
      )
    );
  }
 ...
 * (d) 跨字段一致性（告警级，不阻断注册）。
 */
export function checkCrossField(cap: CdpCapability): CdpValidationError[] {
  const errors: CdpValidationError[] = [];
  if (
    cap.runtime.side_effects.level === 'none' &&
    cap.runtime.side_effects.scope.length > 0
  ) {
    errors.push(
      new CdpValidationError(
        'capability.runtime.side_effects.scope',
        '声明 side_effects.level=none 却列出 scope',
        'L2'
      )
    );
  }
  if (cap.identity.archetype !== cap.cognitive_style.archetype) {
    errors.push(
      new CdpValidationError(
        'capability.cognitive_style.archetype',
        `与 identity.archetype("${cap.identity.archetype}") 不一致`,
        'L2'
      )
    );
  }
 ...
 */
export function validateSemantics(cap: CdpCapability): CdpValidationError[] {
  return [
    ...checkConflicts(cap),
    ...checkTagHints(cap),
    ...checkLatency(cap),
 ...
 */
// TODO L3: interface CdpBehaviorOracle { verify(cap, against): Promise<VerificationReport> }
